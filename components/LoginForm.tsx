...
export default function LoginForm() {
  ...
  // Read the tab straight from the URL on the first render — no useEffect, so
  // the correct tab shows immediately with no flash. `?mode=signup` comes from
  // the header "Sign Up" button.
  const [mode, setMode] = useState<Mode>(
    searchParams.get("mode") === "signup" ? "signup" : "signin"
  );
  // Notices from ?confirmed / ?notice are seeded once from the URL — no effect,
  // so there is no flash and no cascading render.
  const noticeKey = searchParams.get("confirmed")
    ? "confirmed"
  ...
  const [showResend, setShowResend] = useState(
    () => noticeKey === "link_used" || noticeKey === "link_expired"
  );
